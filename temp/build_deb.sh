...
function header_version {
  ...
  /PHP_CASSANDRA_MAJOR/ { major=$3 }
  /PHP_CASSANDRA_MINOR/ { minor=$3 }
  /PHP_CASSANDRA_RELEASE/ { patch=$3 }
  END { 
    printf "%s.%s.%s", major, minor, patch 
  }
EOF
  version=$(grep '#define[ \t]\+PHP_CASSANDRA_\(MAJOR\|MINOR\|RELEASE\)' $1 | awk "$version_script")
...
function header_version_full {
  ...
  /PHP_CASSANDRA_MINOR/ { minor=$3 }
  /PHP_CASSANDRA_RELEASE/ { patch=$3 }
  /PHP_CASSANDRA_STABILITY/ { suffix=$3; gsub(/"/, "", suffix) }
  END { 
    if (length(suffix) > 0)
...
EOF
  version=$(grep '#define[ \t]\+PHP_CASSANDRA_\(MAJOR\|MINOR\|RELEASE\|STABILITY\)' $1 | awk "$version_script")
  if [[ ! $version =~ ^[0-9]+\.[0-9]+\.[0-9]+(~[a-zA-Z0-9_\-]+)?$ ]]; then
...
files="config.m4 php_cassandra.c php_cassandra.h php_cassandra_types.h version.h src util"
